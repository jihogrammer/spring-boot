...
@Component
public class ProductUpdatePayloadValidator implements Validator {

    @Override
    public boolean supports(@NonNull final Class<?> clazz) {
        return ProductUpdatePayload.class.isAssignableFrom(clazz);
    }

    @Override
    public void validate(@NonNull final Object target, @NonNull final Errors errors) {
        var request = (ProductUpdatePayload) target;
