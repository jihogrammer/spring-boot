package dev.jihogrammer.product.adaptor.out;
...
@Slf4j
public class InMemoryProductRepository implements Products {
    ...
    public Product save(final ProductSaveCommand command) {
        ...
        if (productOptional.isEmpty()) {
            ...
            updatedAt = null;
            log.trace("Create product. command=[{}]; createdAt=[{}];", command, createdAt);
        } else {
            createdAt = productOptional.get().createdAt();
            updatedAt = LocalDateTime.now();
            log.trace("Update product. command=[{}]; updatedAt=[{}]", command, updatedAt);
        }
